...
export default function ProjectDetailPage({ project, profile, onSendMessage }: {
...
const bottomRef = useRef<HTMLDivElement | null>(null);
  ...
  const handleSendMessage = async (e:FormEvent<HTMLFormElement>) => {
    ...
    setProjectData(prev => ({...prev, messages:[...projectData.messages, 
      {
        iAmSender:true,
        id:"",
        sender:{
  ...
  const statusColor = project.status === PROJECT_STATUS.PENDING
        ? "bg-amber-500"
  ...
  return (
    <div className="relative flex flex-col h-full mx-auto gap-2">
      ...
      {/* ================= MESSAGES ================= */}
      <Card ref={scrollRef} className="flex-1 overflow-y-auto [scrollbar-width:thin]">
        ...
        <div className="w-max text-xs text-muted-foreground flex items-center gap-3 mt-20 mx-auto sticky top-16 rounded-full p-2 bg-neutral-100">
          ...
          {projectData.messages.map((message, index) => {
            return <MessageCard key={message.id + `---${index}`} isFirst={index === 0} message={message} onPreviewTask={() => {
              if (!!message.task) {
                router.push(
                  `/dashboard/my-projects/${project.id}/createTask/editTask/${message.task?.id}`
                );
              }
            }} />
          })}
      ...
      <form onSubmit={handleSendMessage} className="flex items-center gap-2">
        ...
        <Button
          ...
          onClick={() =>
            router.push(
              `/dashboard/my-projects/${project.id}/createTask`
            )
          }
        >
          <PlusCircle size={18} />
